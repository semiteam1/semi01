...
CREATE TABLE USER_INFO
    ...
    , EMAIL VARCHAR2(50)
    , PHONE VARCHAR2(20)
    , GRADE_NO NUMBER NOT NULL CONSTRAINT FK_USER_GRADE_NO REFERENCES GRADE
...
CREATE TABLE PRODUCT 
    ...
    , PRODUCT_LEVEL NUMBER NOT NULL CONSTRAINT FK_PRODUCT_LEVEL_INFO REFERENCES CATEGORY_INFO
    , RUN_TIME NUMBER NOT NULL
    , PRICE NUMBER NOT NULL
    );
COMMENT ON COLUMN PRODUCT.PRODUCT_NO IS '상품번호';
COMMENT ON COLUMN PRODUCT.CATEGORY_TYPE IS '컨텐츠타입';
...
CREATE TABLE PRODUCT_LIKE
    ...
    , USER_NO NUMBER CONSTRAINT FK_PRODUCT_USER_LIKE_NO REFERENCES PRODUCT
    );
...
CREATE TABLE SCREENING_INFO
    (
      PRODUCT_NO NUMBER CONSTRAINT FK_SCREENING_INFO REFERENCES PRODUCT CONSTRAINT PK_SCREENING_INFO PRIMARY KEY
    , SCREENING_DATE DATE NOT NULL
    , SCREENING_TIME NUMBER NOT NULL
    , SCREENING_SEAT NUMBER
    );        
COMMENT ON COLUMN SCREENING_INFO.PRODUCT_NO IS '상품번호';
COMMENT ON COLUMN SCREENING_INFO.SCREENING_DATE IS '해당날짜';
COMMENT ON COLUMN SCREENING_INFO.SCREENING_TIME IS '오전1/오후2';
COMMENT ON COLUMN SCREENING_INFO.SCREENING_SEAT IS '잔여좌석';

-- 상품이미지 테이블
CREATE TABLE PRODUCT_IMG
(
 PRODUCT_NO NUMBER CONSTRAINT FK_PRODUCT_NO_INFO REFERENCES PRODUCT CONSTRAINT PK_PRODUCT_NO_INFO PRIMARY KEY,
 POSTER_NAME VARCHAR2(300),
 POSTER_PATH VARCHAR2(1000),
 DETAIL1_NAME VARCHAR2(300),
 DETAIL1_PATH VARCHAR2(1000),
 DETAIL2_NAME VARCHAR2(300),
 DETAIL2_PATH VARCHAR2(1000),
 DETAIL3_NAME VARCHAR2(300),
 DETAIL3_PATH VARCHAR2(1000),
 DETAIL4_NAME VARCHAR2(300),
 DETAIL4_PATH VARCHAR2(1000),
 DETAIL5_NAME VARCHAR2(300),
 DETAIL5_PATH VARCHAR2(1000)
);
COMMENT ON COLUMN PRODUCT_IMG.PRODUCT_NO IS '상품번호';
COMMENT ON COLUMN PRODUCT_IMG.POSTER_NAME IS '포스터 이름';
COMMENT ON COLUMN PRODUCT_IMG.POSTER_PATH IS '포스터 경로';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL1_NAME IS '상세1 이름';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL1_PATH IS '상세1 경로';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL2_NAME IS '상세2 이름';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL2_PATH IS '상세2 경로';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL3_NAME IS '상세3 이름';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL3_PATH IS '상세3 경로';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL4_NAME IS '상세4 이름';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL4_PATH IS '상세4 경로';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL5_NAME IS '상세5 이름';
COMMENT ON COLUMN PRODUCT_IMG.DETAIL5_PATH IS '상세5 경로';
...
CREATE TABLE REVIEW_INFO
    (
      REVIEW_NO NUMBER CONSTRAINT FK_REVIEW_INFO_NO REFERENCES PRODUCT CONSTRAINT PK_REVIEW_INFO_NO PRIMARY KEY
    , PRODUCT_NO NUMBER CONSTRAINT FK_REVIEW_PRODUCT_NO REFERENCES PRODUCT
    , REVIEW_CONTENT VARCHAR2(1000) NOT NULL
    , USER_NO NUMBER NOT NULL CONSTRAINT FK_REVIEW_USER_NO REFERENCES USER_INFO
    , REVIEW_DATE DATE NOT NULL
    );
COMMENT ON COLUMN REVIEW_INFO.REVIEW_NO IS '리뷰번호';
COMMENT ON COLUMN REVIEW_INFO.PRODUCT_NO IS '관람한 상품 번호';
